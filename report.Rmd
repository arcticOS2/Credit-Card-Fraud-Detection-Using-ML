...
par(mfrow = c(2,3))
{
  for (i in 1:30) {
    if (i == 30) {
      ...
      plot(x_val, data$Class,pch = 20, xlab = 'Serial Number', ylab = 'Class', main = 'Combined Effect')
      class_pred <- as.vector(predict(lm(Class ~., data = data)))
      abline(lsfit(x_val, class_pred), col = 'red')
      break
    }
    plot(data[,i],data$Class,pch = 20, xlab = x_axis[i], ylab = 'Class')
...
## Applying SMOTE
SMOTE, or Synthetic Minority Over-sampling Technique, is a method used to address class imbalance in classification problems. Instead of simply duplicating samples from the minority class, it generates synthetic examples. It does this by selecting a minority class sample, finding its nearest neighbors, and creating a new synthetic point along the line between the sample and one of its neighbors. This helps increase the representation of the minority class in a more generalized way, reducing the risk of overfitting that comes with random oversampling. However, SMOTE may sometimes introduce noise if the classes overlap too much.
```{r}
smote_out <- SMOTE(X = data[,-30], target = data$Class, K = 5, dup_size = 30)   #Applying SMOTE method
data1 <- as.data.frame(smote_out$data)     #This over sampled data is our new data that we will be working with
head(data1)
```
...
```{r echo=FALSE}
barplot(table(data1$class),col=c('yellow','red'),main='Barplot to Understand Blanced Data',
...
data1[,-30] <- data1[,-30] %>% 
  scale()
head(data1)      #Checking the first 6 lines of the edited data
```
...
split=sample.split(data1$class,SplitRatio=0.7)     #Including the data into splitting sets randomly
training_set=subset(data1,split==TRUE)
testing_set=subset(data1,split==FALSE)
barplot(table(training_set$class))
head(training_set)
...
$$
Accuracy = 1 - \frac{\text{Total Number of Incorrect Predictions}}{\text{Total Number of Predictions}}
$$
...
**ME for J index**
```{r}
Accuracy=sum(predict_df1[,1]==predict_df1[,2])/length(predict_df1[,2])
Accuracy*100
```
...
```{r}
Accuracy2=sum(predict_df2[,1]==predict_df2[,2])/length(predict_df2[,2])
Accuracy2*100
```
**Comment** Clearly the Youden's J statistic is a lesser accuracy error of 97.8%. But, accuracy of 99% using cutpoint == 0.5 is not practically sweetable. So, we choose Youden's J statistic.
...
**Imterpretation:** The `ME == 1- ACCURACY == 4.913124%` means out of 100 predictions 4.9 or round of 5 predictions were incorrectly classified. Whereas, from the ROC curve it is seen that the bending of the curve is closer towards 1 that implies the model has a higher accuracy. the `auc.medel1 == 0.9867` means it has a good measure of separability i.e. it is more or less correctly specifies 0 as 0 and 1 as 1.
...
points(sorted.frame$`Actual Class`, col = 'red',pch = 20)
legend('left',legend=c('Actual Class','Predicted Probability'),fill=c('red','blue'))
...
Accuracy_test=((sum(test.data.class[,1]==test.data.class[,3])/length(test.data.class[,1])))*100
Accuracy_test
```
