...
# Data Description
We need to first import the dataset. Then we'll discuss about different feartures of the dataset as below. But, beforehand let us just call the necessary libraries.
...
**Comment:** The `SplitRatio = 0.7` means that we are deviding the whole dataset in a 70-20 percentage where 70% of the data will go to the `training_set` and the remaining 20% will go to the `testing_set`.
...
legend('topright',legend=c('Non-detected (243045)','Detected (216348)'),fill=c('yellow','red'))
```
 
 
 
 
 **Comment:** Out of the total users, 216348 users were detected as fraud using this cut off i.e. `optimal_cutoff = 0.4806774`. The information is visible in the `barplot()` as well.
...
legend('topright',legend=c('Non-detected (244766)','Detected (214453)'),fill=c('yellow','red'))
```
...
ME*100
```
**Comment** Clearly the Youden's J statistic is a lesser missclassification error of 4.91%. So our ultimate `cut point == 0.4806774`.

# Reforming the model using significant predictors
