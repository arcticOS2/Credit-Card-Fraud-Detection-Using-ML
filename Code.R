...
data=read.csv('data.csv')  #Importing the creditcard.csv datas
...
#DATA AUGMENTATION
# From the barplot it is clearly observable that data is highly imbalanced
# We will use random over sampling to do this

library(ROSE)
library(smotefamily)
over.sample_data <- ovun.sample(Class~.,data = data, method = 'over')
...
barplot(table(data$Class),col=c('yellow','red'),main='Barplot to Understand Blanced Data',
        xlab='Detected-Nondetected',ylab='Number Of Users', ylim = c(0,350000))    
legend('topright',legend=c('Non-detected(284315)','Detected(283797)'),fill=c('yellow','red'))
